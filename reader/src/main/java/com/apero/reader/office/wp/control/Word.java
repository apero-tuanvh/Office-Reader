...
public class Word extends LinearLayout implements IWord
{
    ...
    {
        Log.e("onDraw", "Word isStopDraw = " + isStopDraw);
        if (!initFinish || currentRootType == WPViewConstant.PRINT_ROOT || isStopDraw)
        {
            return;
        }
        try
    ...
    public float getFitZoom()
    {
        ...
        else if (currentRootType == WPViewConstant.PAGE_ROOT)
        {
            ...
            pageWidth = view == null ? 0 : view.getWidth();
            if (pageWidth == 0)
            {
        ...
        return Math.min(z, (pageWidth == -1) ? 1 : getDefaultZoom(pageWidth));
    }

    private float getDefaultZoom(int pageWidth) {
        float fullScreenWidth = Resources.getSystem().getDisplayMetrics().widthPixels;
